...
- AI は「最後にまとめてコミット」を基本にしない。意味のある区切りごとにコミット可否を判断する
- 次の作業に進む前に、自分の変更が未コミットで残っていたら先にコミット可否を確認する
- 区切りの目安は次のいずれか
  - 1つの機能追加・修正・文書整理として説明できる
  - テスト、確認、レビューが一通り終わった
  - 次の変更が別 concern に移る
  - これ以上積むとコミットメッセージが曖昧になる
- AI 自走中は、区切りごとに `git status --short` を見て未コミット差分を確認する
- 未完成で壊れた状態は原則コミットしない。スナップショットが必要なときだけ、理由を明記して例外扱いにする
- ユーザーの既存変更や無関係な差分は、自分のコミットに巻き込まない
...
- コミットのタイミングと粒度判断も `.agents/commit-conventions.md` を参照する
